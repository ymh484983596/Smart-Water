                ...
                <transition name="animate__animated animate__bounce" enter-active-class="animate__fadeInUp"
                    ...
                    <div v-show="isShow" class="pwdArea">
                        <div style="flex: 1;display: flex;justify-content: center;align-items: center">
                            用&nbsp;&nbsp;&nbsp;户&nbsp;&nbsp;&nbsp;名:&nbsp;
                            <el-input class="username" v-model="loginUser.name" style="width: 165px"
            ...
            <div id="aaa" class="showInfo" :style="{
                ...
                <transition name="animate__animated animate__bounce" enter-active-class="animate__fadeInUp"
                    ...
                    <div v-show="isShow"
                        ...
                        <!-- 欢迎语 -->
                        <div
                            style="flex: 2;display: flex;align-items: center;font-size: 22px;color: #180404;font-weight: bold">
                            欢迎登入智慧水务监测系统
                        </div>
                        <!-- 欢迎图片 -->
                        <div style="flex: 2">
...
export default {
    ...
    methods: {
        ...
        UserLogin() {
            let user = {};
            user.userName = this.loginUser.name
            user.userPwd = this.loginUser.password
            this.$router.push("/evaluate")
            //要修改
            return
            this.request.post("http://localhost:8085/user/login", user).then(res => {
                if (res.code == "0") {
        ...
        userRegister() {
            if (this.regUser.regUsername === "") {
                ...
                user.userPwd = this.regUser.regPwd
                //要修改
                return
                this.request.post("http://localhost:8085/user/register", user).then(res => {
                    if (res.code == "0") {
